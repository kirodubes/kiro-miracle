...
- `miracle-wm/config.yaml`: `- command: kiro-screenshot region`. The `sh -c` wrapper is dropped — miracle
  execs directly with no shell, which is exactly why a single command works and the old piped one did not.
- **Build `kiro-wayland-dotfiles` first**: it provides the binary this edition's binds call.

### Files Modified
- `etc/skel/.config/miracle-wm/config.yaml`
- `etc/skel/.config/miracle-wm/keybindings.txt`
